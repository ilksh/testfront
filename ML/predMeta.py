"""
SleepState ML – Ensemble over 4 modalities + Posture/Temp meta‑features
---------------------------------------------------------------------
This single file gives you:
  • Step 1  – Synthetic data generator that matches your schema:
              graph g0..g{k-1}, t1_c (°C), t2_c (°C), volume in [0,1], text (paragraph VTT‑like)
              + synthetic posture/temperature meta‑features so the full ensemble can be tested
  • Step 2  – Per‑modality baselines (graph/temp/volume/text/posture_temp) + soft‑vote + stacking ensembles
  • Step 3  – Testing module: 5‑fold CV table, hold‑out report, κ, confusion matrix
  • Real data hooks – load JSON stream with raw FSR arrays, t1_c, t2_c, volume, ts; build epoch‑level
                      posture/temperature meta‑features and feed them to the ensemble

Python 3.9+
Install:  pip install scikit-learn pandas numpy scipy
"""
